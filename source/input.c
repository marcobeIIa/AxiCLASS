...
//            errmsg);
  ...
  /* try to read it */
  class_call(
    parser_read_int(pfc, "N_mscf", &pba->N_mscf, &flag1, errmsg),
    errmsg, errmsg
  );
  printf("pba->N_mscf = %d \n",pba->N_mscf);
  if(pba->N_mscf >0){
    ...
    printf("debug: survived a bit longer \n");
      ...
      // class_alloc(target_indices, _NUM_TARGETS_ * sizeof(int), errmsg);
  ...
  *has_shooting=_FALSE_;
  ...
  for (index_target = 0; index_target < _NUM_TARGETS_; index_target++){
    ...
    if (flag1 == _TRUE_){
      ...
      class_call(input_needs_shooting_for_target(pfc,
                 ...
                 errmsg,
                 errmsg);
      printf("needs_shooting %d", needs_shooting);
  ...
  //   //   printf("updating unknown_parameters_size? no maxions\n");
  //   printf("%s\n",target_namestrings[index_target]);
  //   flag1=_FALSE_;
  //     class_call(parser_read_double(pfc,target_namestrings[index_target],&param1,&flag1,errmsg),
  //               errmsg,
  ...
  //     /* input_needs_shoting_for_target takes care of the case where, for
  //        instance, Omega_dcdmdr is set to 0.0, and we don't need shooting */
  //     printf("updating unknown_parameters_size? calling input needs shooting\n");
  //     class_call(input_needs_shooting_for_target(pfc,
  //                                               index_target,
  //                                               param1,
  //                                               &needs_shooting,
  ...
  //               errmsg,
  //               errmsg);
  //     printf("updating unknown_parameters_size? called input needs shooting, needs_shooting = %d\n", needs_shooting);
  //     } //CHECK obviously
  //     if (needs_shooting == _TRUE_){
  //       printf("i need shooting!!");
  //       if(input_verbose > 10){
  //         printf("Found target: %s, target value =  %e\n",target_namestrings[index_target],param1);
  //       }
  //       if(fzw.do_shooting_scf ==_FALSE_){
  ...
  //         printf("unknown_parameters_size = %d \n",unknown_parameters_size);
  //         fzw.required_computation_stage = MAX(fzw.required_computation_stage,target_cs[index_target]);
  //         printf("required computation stage %d \n",fzw.required_computation_stage);
  ...
  //     printf("mscf unknown parameter size = %d\n",unknown_parameters_size);
  ...
  if (unknown_parameters_size > 0 && fzw.do_shooting == _TRUE_) {
    ...
    *has_shooting=_TRUE_;
    ...
    while (counter < unknown_parameters_size){
      ...
      printf("in shooting zc is zeq: found fzw.log10_axion_ac %e\n", fzw.log10_axion_ac);
      } else if (strcmp(target_namestrings[index_target], "fraction_maxion_ac") == 0 ||
            strcmp(target_namestrings[index_target], "log10_maxion_ac")    == 0) {
        ...
        for (n_mscf = 0; n_mscf < pba->N_mscf; n_mscf++){
          ...
          printf("mscf shooting for target %s with value %e \n",target_namestrings[index_target+n_mscf],param1);
    ...
    /** Otherwise we do multidimensional shooting */
      ...
      if (input_verbose > 0) {
        printf("last thing it be printing\n");
...
int input_get_guess(double *xguess,
  ...
  /** Estimate dxdy */
  printf("entering guess case loop\n");
  printf("pfzw->target_size = %d\n",pfzw->target_size);
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    printf("index_guess = %d\n",index_guess);
    // printf("Target %s: target_value %e guess %e dxdy %e\n",pfzw->target_name[index_guess],pfzw->target_value[index_guess],xguess[index_guess],dxdy[index_guess]);
    // printf("Target %s:\n",pfzw->target_name[index_guess]);
    switch (pfzw->target_name[index_guess]) {
      case theta_s:
      case theta_s_100:
        printf("guessing theta_s_100\n");
      ...
      case omega_dcdmdr:
        ...
        */
        ...
        printf("guessing log10_axion_ac_phi2n\n");
      ...
      case log10_fraction_axion_ac_phi2n:
        printf("guessing log10_fraction_axion_ac_phi2n\n");
      ...
      case fraction_axion_ac:
        printf("guessing fraction_axion_ac\n");
          /*OLD*/
      ...
      case log10_axion_ac:
        printf("guessing log10_axion_ac\n");
           /*OLD*/
           // phi_initial = ba.scf_parameters[0];
      ...
      case Omega_scf_shoot_fa:
        printf("guessing Omega_scf_shoot_fa\n");
          // class_test()
        ...
        printf("done guessing Omega_scf_shoot_fa\n");
           break;

      case Omega_scf:
        printf("guessing Omega_scf\n");
    ...
    * - Version 3: use attractor solution */
    ...
    if(ba.Omega0_dcdm > 0) Omega_m += ba.Omega0_dcdm;
      ...
      case omega_ini_dcdm:
        printf("guessing omega_ini_dcdm\n");
...
/* try to read it */
  ...
  if (pba->N_mscf > 0){
    ...
    // if (flag1 == _TRUE_){
     ...
     class_alloc(pba->log10_m_maxion, pba->N_mscf * sizeof(double), errmsg); //these two are updated in background.c
    ...
    class_read_list_of_doubles_or_default("alpha_squared_mscf", pba->m_mscf,-30., pba->N_mscf)
    ...
    class_read_list_of_doubles_or_default("log10_fraction_maxion_ac",pba->log10_fraction_maxion_ac,-30.,pba->N_mscf);
     for (n_mscf = 0; n_mscf <pba->N_mscf; n_mscf++){//CHECK i am setting these to zero because i have allocated them 
       pba->f_ede_mscf[n_mscf]              =  0.0;   // this guy looks useless
