...
int perturbations_init(
  ...
  if(pba->has_idm == _TRUE_ && ppt->DMDE_interaction != 0){
    //we incorporate IDM-DE interactions
    if(pba->has_fld == _TRUE_){
      //we consider IDM_fld interactions
      ppt->has_idm_fld = _TRUE_;
    }
    if(pba->has_scf == _TRUE_){
     //we consider IDM_scf interactions
      ppt->has_idm_scf = _TRUE_;
    }
  }
  ...
  */
  ...
  if(pba->scf_evolve_as_fluid == _TRUE_ && pba->scf_evolve_like_axionCAMB == _FALSE_){
    if (ppt->perturbations_verbose > 0)
      printf(" You are running axion with a switch from scf to fluid approximation; this currently leads to a crash if several cores are used. please make sure to use only one core. To be debugged.\n");
    // number_of_threads = 1;
  }
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      if (pba->has_mscf == _TRUE_){
        for (n_mscf=0; n_mscf < pba->N_mscf; n_mscf++){
          ...
          class_sprintf(tmp,"delta_phi_prime_mscf[%d]", n_mscf);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
          class_sprintf(tmp,"delta_mscf[%d]", n_mscf);
          class_store_columntitle(ppt->scalar_titles,tmp,_TRUE_);
//        if(ppt->use_big_theta_mscf == _TRUE_){
  //        class_sprintf(tmp,"big_theta_mscf[%d]", n_mscf);
   //       class_store_columntitle(ppt->scalar_titles,tmp, _TRUE_);
 //         }
    //    else {
          class_sprintf(tmp,"theta_mscf[%d]", n_mscf);
          class_store_columntitle(ppt->scalar_titles,tmp, _TRUE_);
     //     }
        }
      }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_fld == _TRUE_) {
        ...
        if(ppt->use_new_fld_IC == _TRUE_){
          ...
          /*VP: OLD CLASS INITIAL CONDITIONS */
          if (pba->use_ppf == _FALSE_) {
      ...
      if (pba->has_mscf == _TRUE_ && pba->mscf_has_perturbations == _TRUE_) {
        for (n_mscf = 0; n_mscf < pba->N_mscf; n_mscf++){
          ...
          */
          ...
          // if(pba->mscf_evolve_like_axionCAMB == _FALSE_) {
            ppw->pv->y[ppw->pv->index_pt_phi_mscf+n_mscf] = 0.;//a*a/k/k/ppw->pvecback[pba->index_bg_phi_prime_scf]*k*ktau_three/4.*1./(4.-6.*(1./3.)+3.*1.) * (ppw->pvecback[pba->index_bg_rho_scf] + ppw->pvecback[pba->index_bg_p_scf])* ppr->curvature_ini * s2_squared;
            ppw->pv->y[ppw->pv->index_pt_phi_prime_mscf+n_mscf] = 0.;//a*a/ppw->pvecback[pba->index_bg_phi_prime_scf]*( - ktau_two/4.*(1.+1./3.)*(4.-3.*1.)/(4.-6.*(1/3.)+3.*1.)*ppw->pvecback[pba->index_bg_rho_scf] - ppw->pvecback[pba->index_bg_dV_scf]*ppw->pv->y[ppw->pv->index_pt_phi_scf])* ppr->curvature_ini * s2_squared;
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_scf == _TRUE_ ) {
        if(pba->scf_potential == axionquad || (pba->scf_potential==axion && pba->n_axion == 1)){
      ...
      /*for simplicity ignore the following -- relevant only for n_axion = 1 which is not our inetersting case 1!!*/

      // if (pba->has_mscf == _TRUE_ ) {
      //   for (n_mscf = 0; n_mscf < pba->N_mscf; n_mscf++){
      //   // if(pba->scf_potential == axionquad || (pba->scf_potential==axion && pba->n_axion == 1)){
      //   if(pba->n_axion_mscf[n_mscf] == 1){
      //     delta_cdm += ppw->pvecback[pba->index_bg_rho_mscf+n_mscf] * ppw->pv->y[ppw->pv->index_pt_delta_mscf+n_mscf];
      //     rho_cdm += ppw->pvecback[pba->index_bg_rho_mscf+n_mscf];
      //   // }
      //     }
      //   }
      ...
      if ((pba->has_fld == _TRUE_) && (pba->use_ppf == _FALSE_)) {
        ...
        if(ppt->use_delta_fld_over_1plusw==_TRUE_)ppw->pv->y[ppw->pv->index_pt_delta_fld] -= 3*a_prime_over_a*alpha;
        else ppw->pv->y[ppw->pv->index_pt_delta_fld] -= 3*(1.+w_fld)*a_prime_over_a*alpha;
      ...
      if (pba->has_mscf == _TRUE_&& pba->mscf_has_perturbations == _TRUE_) {
        ...
        for (n_mscf = 0; n_mscf < pba->N_mscf; n_mscf++){
          alpha_prime = 0.0;
            /* - 2. * a_prime_over_a * alpha + eta
              - 4.5 * (a2/k2) * ppw->rho_plus_p_shear; */
          ppw->pv->y[ppw->pv->index_pt_phi_mscf+n_mscf] += alpha*ppw->pvecback[pba->index_bg_phi_prime_mscf+n_mscf];
          ppw->pv->y[ppw->pv->index_pt_phi_prime_mscf+n_mscf] +=
          (-2.*a_prime_over_a*alpha*ppw->pvecback[pba->index_bg_phi_prime_mscf+n_mscf]
          -a*a* dV_scf(pba,ppw->pvecback[pba->index_bg_phi_mscf+n_mscf])*alpha
          +ppw->pvecback[pba->index_bg_phi_prime_mscf+n_mscf]*alpha_prime);
          //  }
        }
      }
...
int perturbations_total_stress_energy(
  ...
  int index_q,n_ncdm,n_mscf,idx, index_k;
  ...
  if (_scalars_) {
    ...
    */
    ...
    if(pba->has_scf == _TRUE_ && ppt->include_scf_in_delta_m == _TRUE_ && ppt->include_scf_in_delta_cb == _FALSE_){
        if (ppt->has_source_delta_m == _TRUE_) {
            delta_rho_m += delta_rho_scf; // contribution to delta rho_matter
            rho_m += ppw->pvecback[pba->index_bg_rho_scf];
        }
    ...
    if (pba->has_mscf == _TRUE_&& pba->mscf_has_perturbations == _TRUE_) {
      ...
      for (n_mscf = 0; n_mscf < pba->N_mscf; n_mscf++){
        if(ppt->gauge == synchronous){
            ...
            /* recover these lines as soon as log10_axion_ac is implemented into mscf
               (shouldn't matter because we care about n_axion_mscf ~ 3 which shouldn't give
               the numerial bug but ask) */
            //  if(pba->n_axion < pba->n_axion_security && ppw->pvecback[pba->index_bg_Omega_scf]<pba->security_small_Omega_scf && a>pow(10,pba->log10_axion_ac)){
            //    /*at late times, for small values of n, when the field becomes negligible we ignore its impact to avoid numerical bug */
            //    delta_rho_scf=0;
            //    delta_p_scf=0;
      ...
      // delta_rho_scf=0;
      // delta_p_scf=0;
      ppw->delta_rho += delta_rho_mscf;

      ppw->delta_p += delta_p_mscf; // an important difference from CDM because if SCF evolving via KG then pressure is not necessarily zero.
        // if(pba->n_axion < pba->n_axion_security && ppw->pvecback[pba->index_bg_Omega_scf]<pba->security_small_Omega_scf && a>pba->a_c){
        //   //do nothing
        // }
        // else{
      ...
      if(ppt->include_mscf_in_delta_cb == _TRUE_){
        ...
        if we only want to include it inside of delta_m, it will be done later */
        if (ppt->has_source_delta_m == _TRUE_) {
            delta_rho_m += delta_rho_mscf; // contribution to delta rho_matter
            rho_m += ppw->pvecback[pba->index_bg_rho_mscf+n_mscf];
        }
        if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
            rho_plus_p_theta_m += rho_plus_p_theta_mscf; // contribution to [(rho+p)theta]_matter
            rho_plus_p_m += (ppw->pvecback[pba->index_bg_rho_mscf+n_mscf]+ppw->pvecback[pba->index_bg_p_mscf+n_mscf]);
        }
      }
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_) {
       ...
       /((1 + pow(pba->a_c/a,3*(1 + pba->w_fld_f)/pba->nu_fld))*pba->nu_fld*(1 + pba->w_fld_f + pow(pba->a_c/a,3*(1 + pba->w_fld_f)/pba->nu_fld)*(1 + pba->w_fld_i)));
        ...
        else  ppw->delta_rho_fld = ppw->pvecback[pba->index_bg_rho_fld]*y[ppw->pv->index_pt_delta_fld];

        if(ppt->use_big_theta_fld == _TRUE_) ppw->rho_plus_p_theta_fld = ppw->pvecback[pba->index_bg_rho_fld]*y[ppw->pv->index_pt_big_theta_fld];
        else ppw->rho_plus_p_theta_fld = (1.+w_fld)*ppw->pvecback[pba->index_bg_rho_fld]*y[ppw->pv->index_pt_theta_fld];
...
int perturbations_print_variables(double tau,
  ...
  double delta_rho_mscf=0, rho_plus_p_theta_mscf=0;
  double delta_scf=0., theta_scf=0., delta_phi_scf,delta_phi_over_phi_scf, delta_phi_prime_scf;
  double big_theta_scf;
  double *delta_mscf=NULL, *theta_mscf=NULL, *delta_phi_mscf=NULL, *delta_phi_over_phi_mscf, *delta_phi_prime_mscf=NULL;
  ...
  if (_scalars_) {
    ...
    if (pba->N_mscf > 0) {
      class_alloc(delta_mscf, pba->N_mscf * sizeof(double), pba->error_message);
      class_alloc(theta_mscf, pba->N_mscf * sizeof(double), pba->error_message);
      class_alloc(delta_phi_mscf, pba->N_mscf * sizeof(double), pba->error_message);
      class_alloc(delta_phi_over_phi_mscf, pba->N_mscf * sizeof(double), pba->error_message);
    ...
    if(pba->has_fld == _TRUE_){
     ...
     /((1 + pow(pba->a_c/a,3*(1 + pba->w_fld_f)/pba->nu_fld))*pba->nu_fld*(1 + pba->w_fld_f + pow(pba->a_c/a,3*(1 + pba->w_fld_f)/pba->nu_fld)*(1 + pba->w_fld_i)));
      ...
      else delta_fld = y[ppw->pv->index_pt_delta_fld];
      if(ppt->use_big_theta_fld == _TRUE_) {
    ...
    if ((ppt->has_mscf==_TRUE_)&&(pba->mscf_has_perturbations == _TRUE_)){
      for (n_mscf=0; n_mscf < pba->N_mscf; n_mscf++){
        ...
        class_store_double(dataptr, delta_mscf[n_mscf], _TRUE_, storeidx);
        class_store_double(dataptr, theta_mscf[n_mscf], _TRUE_, storeidx);
      }
    }
...
int perturbations_derivs(double tau,
  ...
  int tight_coupling_DMEDE=_FALSE_;
  ...
  double f_norm=0;
    ...
    if(pba->has_scf == _TRUE_ && pba->scf_has_perturbations == _TRUE_){
      ...
      if(ppt->has_idm_scf == _TRUE_ && pba->has_scf == _TRUE_){
        // printf("f_ede_peak %e \n", pba->f_ede);
                    if(ppt->scales_like_fEDE == _TRUE_ || ppt->scales_like_fEDE_over_k2 == _TRUE_){
    ...
    if (pba->has_idm == _TRUE_){
      ...
      if (ppt->has_idm_scf == _TRUE_){
        ...
        if(ppt->use_big_theta_scf == _TRUE_){
         ...
         if(1+pvecback[pba->index_bg_w_scf]!=0.0)  dy[pv->index_pt_theta_idm] += ppt->DMDE_interaction*f_norm/pvecback[pba->index_bg_rho_idm]*(y[pv->index_pt_big_theta_scf]/(1+pvecback[pba->index_bg_w_scf])-theta_idm); /* new interaction between DE and cdm */
         else dy[pv->index_pt_theta_idm] += 0.0;
        }else{
...
//            dy[pv->index_pt_theta_cdm] += ppt->DMDE_interaction*a/ppw->pvecback[pba->index_bg_rho_cdm]*(y[pv->index_pt_theta_fld]-y[pv->index_pt_theta_cdm]); /* new interaction between DE and cdm */
          dy[pv->index_pt_theta_idm] += ppt->DMDE_interaction*f_norm/pvecback[pba->index_bg_rho_idm]*(y[pv->index_pt_theta_scf]-theta_idm); /* new interaction between DE and cdm */
        }
        }
        else{
          /* do nothing for now */
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        if(ppt->use_big_theta_fld == _TRUE_){
          dy[pv->index_pt_big_theta_fld] =
            -(1.-3.*cs2)*a_prime_over_a*y[pv->index_pt_big_theta_fld]
            +(1+w_fld)*metric_euler
            + 3*a_prime_over_a*(w_fld-ca2)*y[pv->index_pt_big_theta_fld];
          if(ppt->use_delta_fld_over_1plusw == _TRUE_){
           dy[pv->index_pt_big_theta_fld]  += cs2*k2*(1+w_fld)*y[pv->index_pt_delta_fld];
          }
          else{
           dy[pv->index_pt_big_theta_fld]  += cs2*k2*y[pv->index_pt_delta_fld];
          }
          if(ppt->has_idm_fld==_TRUE_ && (1+w_fld)!= 0.0)dy[pv->index_pt_big_theta_fld] -= ppt->DMDE_interaction*f_norm/pvecback[pba->index_bg_rho_idm]*(pvecback[pba->index_bg_rho_idm]/((1+w_fld)*pvecback[pba->index_bg_rho_fld]))*(y[pv->index_pt_big_theta_fld]/(1+w_fld)-y[pv->index_pt_theta_idm]); /* cdm velocity */
...
//            if(pba->has_cdm==_TRUE_ && ppt->DMDE_interaction > 0)dy[pv->index_pt_theta_fld] -= ppt->DMDE_interaction*a/ppw->pvecback[pba->index_bg_rho_cdm]*(ppw->pvecback[pba->index_bg_rho_cdm]/((1+w_fld)*ppw->pvecback[pba->index_bg_rho_fld]))*(y[pv->index_pt_theta_fld]-y[pv->index_pt_theta_cdm]); /* cdm velocity */
          ...
          // if(ppt->DMDE_interaction*f_norm/pvecback[pba->index_bg_rho_idm]*(pvecback[pba->index_bg_rho_idm]/((1+w_fld)*pvecback[pba->index_bg_rho_fld]))/a_prime_over_a >1){
          //   y[pv->index_pt_theta_fld] = y[pv->index_pt_theta_idm];
          // }
        }
      }
    ...
    if (pba->has_scf == _TRUE_ && pba->scf_has_perturbations == _TRUE_) {
      ...
      if(pba->scf_evolve_as_fluid == _TRUE_) {
        ...
        if(ppt->use_delta_scf_over_1plusw == _TRUE_){
          ...
          dy[pv->index_pt_delta_scf] = -3.*(cs2-pvecback[pba->index_bg_w_scf])*a_prime_over_a*y[pv->index_pt_delta_scf];
          // switch depending on whether or not big_theta_fld is being used, terms in continuity eq of delta_fld differ
          if(ppt->use_big_theta_scf == _TRUE_){
      ...
      if(ppt->use_big_theta_scf == _TRUE_){
        ...
        if(ppt->use_delta_scf_over_1plusw == _TRUE_){
         dy[pv->index_pt_big_theta_scf]  += cs2*k2*(1+pvecback[pba->index_bg_w_scf])*y[pv->index_pt_delta_scf];
        }
        else{
         dy[pv->index_pt_big_theta_scf]  += cs2*k2*y[pv->index_pt_delta_scf];
        }

        if(ppt->has_idm_scf==_TRUE_ && 1+pvecback[pba->index_bg_w_scf] != 0.0)dy[pv->index_pt_big_theta_scf] -= ppt->DMDE_interaction*f_norm/ppw->pvecback[pba->index_bg_rho_idm]*(pvecback[pba->index_bg_rho_idm]/pvecback[pba->index_bg_rho_scf])*(y[pv->index_pt_big_theta_scf]/(1+pvecback[pba->index_bg_w_scf])-y[pv->index_pt_theta_idm]); /* cdm velocity */
    ...
    if (pba->has_mscf == _TRUE_ && pba->mscf_has_perturbations == _TRUE_) {
      for (n_mscf = 0; n_mscf < pba->N_mscf; n_mscf++) {
        ...
        /** - ----> field value */
        dy[pv->index_pt_phi_mscf+n_mscf] = y[pv->index_pt_phi_prime_mscf+n_mscf];
        if(ppt->perturbations_verbose>11){
          fprintf(stdout,"Passed 'field value calc' for %d -th field \n ", n_mscf);
        }
        /** - ----> Klein Gordon equation */
        ...
        if(ppt->perturbations_verbose>11){
        printf("%d -th field, k %e a %e delta_phi %e delta_phi' %e d_delta_phi' %e metric_continuity %e\n",n_mscf,k,a, dy[pv->index_pt_phi_mscf+n_mscf],y[pv->index_pt_phi_prime_mscf+n_mscf],dy[pv->index_pt_phi_prime_mscf+n_mscf],metric_continuity);
        printf("phi' %e ddV %e\n",pvecback[pba->index_bg_phi_prime_mscf+n_mscf],pvecback[pba->index_bg_ddV_mscf+n_mscf]);
        }
        // if(ppt->perturbations_verbose>11){
          // fprintf(stdout,"Passed 'KG calc' \n ");
        // }
